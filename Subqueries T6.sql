...
-- find the No. of peoples in each county 
SELECT c.Country_name, COUNT(p.Id) AS person_count
FROM Country_CC c
JOIN Persons_PP p ON c.Id = p.Country_Id
GROUP BY c.Country_name;

-- find the number persone in each country sorted from high to low
SELECT c.Country_name, COUNT(p.Id) AS person_count
FROM Country_CC c
JOIN Persons_PP p ON c.Id = p.Country_Id
GROUP BY c.Country_name
...
-- To find the average rating for a person in respective countries if the average is greater than 3.0.
...
HAVING AVG(p.Rating) = (
    ...
    FROM Persons_PP p2
    JOIN Country_CC c2 ON p2.Country_Id = c2.Id
    WHERE c2.Country_name = 'USA'
);

-- To find the countries with same rating as USA
SELECT c.Country_name
FROM Country_CC c
JOIN Persons_PP p ON c.Id = p.Country_Id
GROUP BY c.Country_name
...
-- To select all country whose population is greater than the average population of all nations? 
SELECT Country_name
FROM Country_CC
WHERE Population > (
